  ...
  module AggregateStore
    ...
    module ClassMethods
      def aggregate_attribute(name, class_name, options = {})
        aggregated_attributes[name] = (agg_attribute = Aggregate::AttributeHandler.factory(name, class_name, options))
      ...
      def aggregate_has_many(name, class_name, options = {})
        aggregated_attributes[name] = (agg_attribute = Aggregate::AttributeHandler.has_many_factory(name, class_name, options))
      ...
      def aggregate_belongs_to(name, options = {})
        aggregated_attributes[name] = (agg_attribute = Aggregate::AttributeHandler.belongs_to_factory("#{name}_id", options))
      ...
      def aggregate_schema_version(version_number, update_callback)
        aggregated_attributes[name] = (agg_attribute = Aggregate::Attribute::SchemaVersion.new(version_number, update_callback))
    ...
    def to_store
      self.class.aggregated_attributes.build_hash do |_, aa|
    ...
    def validate_aggregates
      self.class.aggregated_attributes.each do |_, aa|
    ...
    def inspect_aggregates(level = 1)
      self.class.aggregated_attributes.map do |_, aa|
    ...
    def aggregate_attributes
      self.class.aggregated_attributes.build_hash do |_, attr|
