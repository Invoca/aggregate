...
class Aggregate::BaseTest < ActiveSupport::TestCase
  ...
  context "Aggregate classes" do
    ...
    should "allow instances to be initialized from the constructor" do
      @instance = @agg.new( :name => "Bob", :address => "1812 clearview", :zip => 93101)
      assert_equal "Bob", @instance.name
      assert_equal "1812 clearview", @instance.address
      assert_equal 93101, @instance.zip

      assert @instance.new_record?
    end

    should "allow an instance to be initialized from a store" do
      decoded_aggregate_store = { "name" => "Bob", "address" => "1812 clearview", "zip" => 93101 }
    ...
    should "provide comparable methods for instances" do
      @first  = @agg.new( :name => "Bob", :address => "1812 clearview", :zip => 93101)
      @same   = @agg.new( :name => "Bob", :address => "1812 clearview", :zip => 93101)
      @second = @agg.new( :name => "Bob", :address => "1812 clearview", :zip => 93102)
    ...
    should "support comparison for nested instances" do
      ...
      @first = @outer_agg.new(
        :type => :tiger,
        :istrue => false,
        :address1 => @agg.new( :name => "Bob", :address => "1812 clearview", :zip => 93101),
        :address2 => @agg.new( :name => "Murphy", :address => "1414 mountain", :zip => 93103)
      )
      ...
      assert @first == @second
      @second.address2.zip = 93102
      assert @first != @second
      assert @first > @second

      @second.address2.zip = 93103
      assert @first == @second
